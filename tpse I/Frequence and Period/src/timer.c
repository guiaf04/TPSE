...
void DMTimerSetUp(void){
    ...
    interrupt_setup(95);//(95 --> Bit 31 do 3º registrador (MIR CLEAR2))
    #endif
}
...
void disableWdt(void){
	HWREG(WDT_WSPR + WDT_BASE) = 0xAAAA;
	while((HWREG(WDT_WWPS + WDT_BASE) & (1<<4)));
	
	HWREG(WDT_WSPR + WDT_BASE) = 0x5555;
	while((HWREG(WDT_WWPS + WDT_BASE) & (1<<4)));
}
