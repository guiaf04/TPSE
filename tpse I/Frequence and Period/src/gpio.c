...
void interrupt_buttom(gpioMod g, ucPinNumber p){
	
	switch (g)
	{
	case GPIO0:
		gpioPinMuxSetup(GPIO0, p);
	
		flag_gpio = false;

		/* Setting interrupt GPIO pin. */
		HWREG(SOC_GPIO_0_REGS + GPIO_IRQSTATUS_SET_0) |= 1<<p; 	

		/* Enable interrupt generation on detection of a rising edge.*/
		HWREG(SOC_GPIO_0_REGS + GPIO_RISINGDETECT) |= 1<<p;	
		break;
	case GPIO1:
		gpioPinMuxSetup(GPIO1, p);
	
		flag_gpio = false;

		/* Setting interrupt GPIO pin. */
		HWREG(SOC_GPIO_1_REGS + GPIO_IRQSTATUS_SET_0) |= 1<<p; 	

		/* Enable interrupt generation on detection of a rising edge.*/
		HWREG(SOC_GPIO_1_REGS + GPIO_RISINGDETECT) |= 1<<p;
		break;
	case GPIO2:
		gpioPinMuxSetup(GPIO2, p);
	
		flag_gpio = false;

		/* Setting interrupt GPIO pin. */
		HWREG(SOC_GPIO_2_REGS + GPIO_IRQSTATUS_SET_0) |= 1<<p; 	

		/* Enable interrupt generation on detection of a rising edge.*/
		HWREG(SOC_GPIO_2_REGS + GPIO_RISINGDETECT) |= 1<<p;
		break;
	case GPIO3:
		gpioPinMuxSetup(GPIO3, p);
	
		flag_gpio = false;

		/* Setting interrupt GPIO pin. */
		HWREG(SOC_GPIO_3_REGS + GPIO_IRQSTATUS_SET_0) |= 1<<p; 	

		/* Enable interrupt generation on detection of a rising edge.*/
		HWREG(SOC_GPIO_3_REGS + GPIO_RISINGDETECT) |= 1<<p;
		break;		
	default:
		break;
	}
